...
/*
    Не хотелось генерить кучу одноразовых DTO, большая часть, которых вообше не используется.
    Думаю это не чать задания, если конечно не ожидалось увидеть маппинг с модели данных на модель API.
    По этому частично сделал через JsonDocument. JsonSerializer используется только для десериализации Office
    Менее затрано с точки зрения использования памяти и быстрее в данном контесте,
    когда не надо маппить весь документ на модель, по стравнению с использованию JsonSerializer ждя всего документа.
*/
...
internal sealed class DataSourceService() : IDataSourceService
{
    ...
    #region IDataSourceService Impemntation
                ...
                foreach (var cityElement in cityElements.EnumerateArray())
                {
                    ...
                    foreach (var terminalElement in terminalsElements.EnumerateArray())
                    {
                        ...
                        using (var stream = new MemoryStream(Encoding.UTF8.GetBytes(rawText)))
                        {
                            ...
                            // Не совсем понятно какое поле содержит область
                            office.AddressRegion = cityElement.GetPropertyOrNull("name")?.GetString();
